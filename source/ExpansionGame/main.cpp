...
public:
	...
	{
		// ¬ыводим количество необходимых расширений дл€ объекта
		std::cout << "Required " << Object << " Extension Count - " << ExtensionCount << ":" << std::endl;
		// ¬ыводим необходимые расширени€ дл€ объекта
...
private:
	void initWindow()
	{
		...
		glfwInit();
		// “ак как GLFW изначально была создана дл€ контекста OpenGL, нам необходимо сказать, чтобы этот контекст не создавалс€ следующим вызовом:
	...
	void mainLoop()
	{
		// ѕока не встретим исключение или не закроем окно, крутим цикл:
		while (!glfwWindowShouldClose(window))
		{
			// ќбрабатывает событи€ окна и ввода.
			glfwPollEvents();
		}
	}
	void cleanup()
	...
	void createLogicalDevice()
	{
		...
		// ƒаже если очередь одна, нужно об€зательно указать приоритет (от 0 до 1)
		...
		// —начала добавл€ем структуры, описанные выше, и только потом остальную часть:
		...
		// ¬ключение расширений устройства:
	...
	void createSwapChain()
	{
		...
		// —тоит указывать как минимум на одно изображение больше минимума, которые будут в swap chain
		...
		// —труктура дл€ создани€ swap chain
		VkSwapchainCreateInfoKHR createInfo{};
		createInfo.sType = VK_STRUCTURE_TYPE_SWAPCHAIN_CREATE_INFO_KHR;
		createInfo.surface = surface;
		// “олько после указани€ поверхности:
		...
		// ”казываем, как обрабатывать изображени€ swap chain, которые будут использовать в нескольких семействах очередей.
		QueueFamilyIndices indices = Utils::findQueueFamilies(physicalDevice, surface);
		uint32_t queueFamilyIndices[] = { indices.graphicsFamily.value(), indices.presentFamily.value() };

		if (indices.graphicsFamily != indices.presentFamily) 
		{
			// изображени€ могут использоватьс€ в нескольких семействах очередей без €вной передачи права владени€.
		...
		{
			// изображение принадлежит только одному семейству очередей в данный момент, и право владени€ должно быть €вно передано перед использованием изображени€ в дургом семействе очередей.
			// Ётот режим обеспечивает наивышую производительность.
	...
	VkPhysicalDevice physicalDevice = VK_NULL_HANDLE; // удал€етс€ не€вно при удалии VkInstance, поэтому можно не заниматьс€ его удалением.
	VkDevice device;
